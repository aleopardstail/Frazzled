...
void PCA9685_servo::loop(uint32_t TEllapsed)
{
	switch (_mode)
	{
		case MODE_FAST:
		{
			if (_isMoving)
			{
				...
				if (_Tcntr >= _TFastDuration)
				{
					...
					_isMoving = 0;
					if (onStopMove != NULL) onStopMove(this);
				}
			}
			...
			if (_isMoving)
			{
				...
				if (_Tcntr >= _TConstantPeriod)
				{
					...
					if (_currentAngle < _targetAngle)
					{
						...
						uint16_t PWM = map(_A, -90, 90, _PWMmin, _PWMmax);
...
void PCA9685_servo::setPosition(int8_t Angle)
{
	switch (_mode)
	{
		...
		case MODE_FAST:
		{
			...
			_isMoving = 1;
			_Tcntr = 0;
			if (onStartMove != NULL) onStartMove(this);
			break;
		}
		...
		case MODE_TCONSTANT:
		{
			...
			uint32_t AngleDelta = abs(Angle - _currentAngle);		// how many degrees we need to move
...
uint8_t PCA9685_servo::isThrown(void)
{
	return _isThrown;
}

uint32_t PCA9685_servo::getTConstantDuration(void)
{
	return _TConstantDuration;
}

uint32_t PCA9685_servo::getSConstantPeriod(void)
{
...
void PCA9685_servo::setTConstantDuration(uint32_t TConstantDuration)
{
	_TConstantDuration = TConstantDuration;
	return;
}

void PCA9685_servo::setSConstantPeriod(uint32_t SConstantPeriod)	
{
