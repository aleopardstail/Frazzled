...
bool      SensorInterlock = false;      // the value of our sensor interlock
bool      LastSensorInterlock = false;  // its previous state so we can see it its changed.
uint16_t  SensorThreshold = 512;        // the sensor trigger point, values above this will trigger the interlock
...
void loop() {
  ...
  if (Serial.available() > 0)
  {
    ...
    if (C == 'g')
    {
      if (Interlock == false && SensorInterlock == false) // both our interlocks are clear
      {
        CurrentState = 1;
        Changed = true;  
      }  
      else
      {
        ...
        if (SensorInterlock)
        {
          Serial.println("+++++ Sensor Interlock Error +++++");
        }
      }    
    }
    else if (C == 'r')
   ...
   // Read the Sensor - NEW Section
  uint16_t Read_AO = analogRead(SENSOR_AO);       
  bool Read_DO = digitalRead(SENSOR_DO);   

  // check to see if the sensor should interlock or not, is its value below the set threshold?
  if (Read_AO < SensorThreshold)
  {
    SensorInterlock = true;
    CurrentState = 0;           // if the sensor interlock activates we set the state to RED
  } 
  else
  {
    SensorInterlock = false;
  }   

    // has the sensor interlock value changed?
  if (SensorInterlock != LastSensorInterlock)
  {
    Changed = true;
  }
  LastSensorInterlock = SensorInterlock;      // store the value

  // if the specified time period has ellapsed, print the sensors values, both digital and analogue
  if (TNow - TPrintPeriod > TLastPrint)
  {
    Serial.print(Read_DO);
    Serial.print("    ");
    Serial.println(Read_AO);
    TLastPrint = TNow;
  }
